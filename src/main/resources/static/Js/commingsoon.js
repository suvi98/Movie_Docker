  ...
  function  fetching_data() {
    ...
    fetch("./data1.json").then(Response=>{
        ...
        function info(data){
            ...
            console.log(data.moviescoming);
            var images = '';
            images += '<img src="' + data.moviescoming[index].posterurl +  '""/>';
            document.getElementsByClassName('movie_img')[index].innerHTML = images;
            document.getElementsByClassName('movie_id')[index].innerHTML= data.moviescoming[index].id;
            document.getElementsByClassName('movie_title')[index].innerHTML= data.moviescoming[index].title;
            document.getElementsByClassName('movie_date')[index].innerHTML= data.moviescoming[index].releaseDate;
        }
    }
