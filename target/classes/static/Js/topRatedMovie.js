  ...
  function  fetching_data() {
    ...
    fetch("./data1.json").then(Response=>{
        ...
        function info(data){
            ...
            for (let index = 0; index < data.topratedmovies.length; index++) {
            console.log(data.topratedmovies);
            var images = '';
            images += '<img src="' + data.topratedmovies[index].posterurl +  '" style=""/>';
            document.getElementsByClassName('movie_img')[index].innerHTML = images;
            document.getElementsByClassName('movie_id')[index].innerHTML= data.topratedmovies[index].year;
            document.getElementsByClassName('movie_title')[index].innerHTML= data.topratedmovies[index].title;
            document.getElementsByClassName('movie_date')[index].innerHTML= data.topratedmovies[index].releaseDate;
        }
    }
