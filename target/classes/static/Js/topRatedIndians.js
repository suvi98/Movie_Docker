  ...
  function  fetching_data() {
    ...
    fetch("./data1.json").then(Response=>{
        ...
        function info(data){
            ...
            for (let index = 0; index < data.topratedindia.length; index++) {
            console.log(data.topratedindia);
            var images = '';
            images += '<img src="' + data.topratedindia[index].posterurl +  '" style=""/>';
            document.getElementsByClassName('movie_img')[index].innerHTML = images;
            document.getElementsByClassName('movie_id')[index].innerHTML= data.topratedindia[index].year;
            document.getElementsByClassName('movie_title')[index].innerHTML= data.topratedindia[index].title;
            document.getElementsByClassName('movie_date')[index].innerHTML= data.topratedindia[index].releaseDate;
        }
    }
